...
def run(weights=[] ,  # model.pt path(s)
        ...
        name='detect_ship',  # save results to project/name
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            # Print time (inference-only) -프레임별 인식 로그 출력
            LOGGER.info(f'{s}Done. ({t3 - t2:.3f}s)')


            # Stream results - 프레임별 detect 이미지 출력
...
detect_weights = 'runs\\train\\6_class_final\\weights\\best.pt' # 학습 모델 경로
...
def file_open_fun():
  ...
  for file in files:
    ...
    #run([detect_weights],str(file))

    thread_img = threading.Thread(target = run, args = ([detect_weights],str(file)))
...
frm = Frame(root, bg="white", width=720, height=480) # 프레임 너비, 높이 설정
